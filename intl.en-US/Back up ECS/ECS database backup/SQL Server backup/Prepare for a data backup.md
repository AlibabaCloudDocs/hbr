...
You can use Hybrid Backup Recovery \(HBR\) to back up SQL Server databases deployed on Elastic Compute Service \(ECS\) instances and restore these databases as needed. SQL Server Backup provides a more powerful and user-friendly interface in the console. This topic describes the preparations that you must make before a data backup.
...
2.  In the left-side navigation pane, choose **Backup** \> **ECS Database Backup**. Click **SQL Server**. On the page that appears, assign the roles as prompted.
