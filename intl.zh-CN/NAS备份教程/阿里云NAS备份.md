...
-   阿里云NAS备份暂时不支持NAS SMB ACL特性，NAS SMB ACL特性功能详情，请参见[阿里云NAS SMB ACL特性]()。若您在备份SMB NAS时由于权限问题导致备份失败，请使用ECS文件备份的方式进行备份。具体操作，请参见[授予HBR读取SMB NAS文件权限](/intl.zh-CN/NAS备份教程/其他方式备份NAS（以文件方式备份）/使用ECS文件备份形式备份SMB NAS文件/授予HBR读取SMB NAS文件权限.md)。

此外，您还可以使用ECS文件备份方式来备份阿里云NAS。更多信息，请参见[概述](/intl.zh-CN/NAS备份教程/其他方式备份NAS（以文件方式备份）/使用ECS文件备份形式备份NFS NAS文件/概述.md)。
...
4.  在备份文件系统对话框，按照如下说明填写各项参数，然后单击**确定**。
...
2.  在**选择备份**页签，按如下说明填写各项参数，然后单击**下一步**。
    ...
    |已备份的NAS|从已经备份的NAS里面选一个作为要恢复的源NAS。|
    |待恢复的备份|在待恢复备份列表中，选择待恢复的备份。|

3.  在**选择恢复项目**页签，选择**恢复项目**，然后单击**下一步**。
...
4.  在**恢复目的地**页签，选择该地域下的任意一个**文件系统**，单击**下一步**。

5.  在**恢复路径**，填写**恢复路径**。

    您可以直接原路径恢复，也可以恢复至指定的地点，并在恢复路径框中输入一个路径，例如/nas/abc（/代表NAS根目录）。
...
2.  单击**概览**页，选择**有备份库的地域**，例如华东1（杭州），然后单击对应备份库右上角的**跨地域备份**。
    ...
    ![kuadiyu](https://static-aliyun-doc.oss-accelerate.aliyuncs.com/assets/img/zh-CN/4848419161/p267393.png)

3.  在创建镜像备份库页面，选择**地域**，填写**备份库名称**。
