...
-   阿里云NAS备份暂时不支持NAS SMB ACL特性，NAS SMB ACL特性功能详情，请参见[阿里云NAS SMB ACL特性]()。若您在备份SMB NAS时由于权限问题导致备份失败，请使用ECS文件备份的方式进行备份。具体操作，请参见[授予HBR读取SMB NAS文件权限](/cn.zh-CN/NAS备份教程/其他方式备份NAS（以文件方式备份）/使用ECS文件备份形式备份SMB NAS文件/授予HBR读取SMB NAS文件权限.md)。

此外，您还可以使用ECS文件备份方式来备份阿里云NAS。更多信息，请参见[概述](/cn.zh-CN/NAS备份教程/其他方式备份NAS（以文件方式备份）/使用ECS文件备份形式备份NFS NAS文件/概述.md)。
